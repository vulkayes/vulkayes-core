...
pub unsafe trait Transparent {
	...
	fn transmute(self) -> Self::Target where Self: Sized {
		unsafe {
	...
	fn transmute_slice(me: &[Self]) -> &[Self::Target] where Self: Sized {
		unsafe {
			std::mem::transmute(me)
		}
	}

	fn transmute_slice_mut(me: &mut [Self]) -> &mut [Self::Target] where Self: Sized {
		unsafe {
			std::mem::transmute(me)
		}
	}
...
mod test {
	use crate::descriptor::set::update::DescriptorSetWrite;
	...
	fn read_last_byte<T>(val: &T) -> u8 {
		unsafe {
			*(
				(val as *const T).add(1) as *const u8
			).sub(1)
		}
	}
