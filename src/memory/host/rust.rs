...
impl RustHostMemoryAllocator {
	...
	unsafe fn dealloc(&mut self, ptr: *mut u8) {
		...
		log::trace!("Deallocated {} bytes aligned at {} from {:p}", layout.size(), layout.align(), ptr);
	}

	fn lock_init_allocator() -> MutexGuard<'static, RustHostMemoryAllocator> {
		ALLOCATOR_INIT.call_once(|| {
			unsafe {
				ALLOCATOR.as_mut_ptr().write(
					Mutex::new(
						RustHostMemoryAllocator {
		...
		unsafe {
			ALLOCATOR.as_ptr().as_ref().unwrap().lock().unwrap()
		}
	}

	pub(super) unsafe extern "system" fn rust_alloc(_: *mut c_void, size: usize, alignment: usize, _: SystemAllocationScope) -> *mut c_void {
	...
	pub(super) unsafe extern "system" fn rust_realloc(_: *mut c_void, p_original: *mut c_void, size: usize, alignment: usize, _: SystemAllocationScope) -> *mut c_void {
		let mut allocator = Self::lock_init_allocator();
	...
	pub(super) unsafe extern "system" fn rust_free(_: *mut c_void, p_memory: *mut c_void) -> c_void {
		let mut allocator = Self::lock_init_allocator();
	...
	pub(super) unsafe extern "system" fn rust_internal_allocation(
		...
		log::trace!("rust_internal_allocation({:p}, {}, {:?}), {:?}", p_user_data, size, allocation_type, allocation_scope);
	...
	pub(super) unsafe extern "system" fn rust_internal_free(
		...
		log::trace!("rust_internal_free({:p}, {}, {:?}), {:?}", p_user_data, size, allocation_type, allocation_scope);
