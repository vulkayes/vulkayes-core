...
impl<'a, I, CR, P> TryInto<SubpassDescription<'a>> for &'a SubpassDescriptionHolder<I, CR, P>
where
	I: AsRef<[AttachmentReference]>,
	CR: AsRef<[AttachmentReference]>,
	P: AsRef<[u32]>
{
	...
	fn try_into(self) -> Result<SubpassDescription<'a>, Self::Error> {
		SubpassDescription::new(
			self.input_attachments.as_ref().map(AsRef::as_ref),
			self.color_resolve_attachments
				.as_ref()
				.map(|(c, r)| (c.as_ref(), r.as_ref().map(AsRef::as_ref))),
			self.depth_stencil_attachment.as_ref(),
			self.preserve_attachments.as_ref().map(AsRef::as_ref)
		)
	}
}
